function ComputeSaliency_RST(dataset, idxFirst, idxLast)
...
for i=1:length(videos)
    ...
    %==========================================================================
    if (sEnhanceCofig.params{j}.localContrast)
        contrast = 'Local';
    else
        contrast = 'Global';
    end
    
    input =  [saveDir '/' dataset '/' 'Enhancement/Fusion/' sEnhanceCofig.params{j}.method '/' sEnhanceCofig.params{j}.description '/Graphcut/TemporalSaliency/' sFusion.method '/' video_name '/' contrast '/SS_png/sal'];
    output = [saveDir '/' dataset '/' video_name];
    if(~exist(output, 'dir'))
        mkdir(output);
    end
    ...
    file = dir([input '/*.png']);
    for j=1:length(file)
        im = imread([input '/' file(j).name]);
        im = im(:,:,1);
        imwrite(im, [output '/' sprintf('frame_%04d.png',j)]);
    end
    
    rmdir([saveDir '/' dataset '/Enhancement' ] , 's');
    rmdir([saveDir '/' dataset '/Fusion' ] , 's');
    rmdir([saveDir '/' dataset '/MotionFlow' ] , 's');
    rmdir([saveDir '/' dataset '/Objectness' ] , 's');
    rmdir([saveDir '/' dataset '/SpatialRefinement' ] , 's');
    rmdir([saveDir '/' dataset '/SpatialSaliency' ] , 's');
    rmdir([saveDir '/' dataset '/SuperPixel' ] , 's');
    rmdir([saveDir '/' dataset '/TemporalSaliency' ] , 's');
end
