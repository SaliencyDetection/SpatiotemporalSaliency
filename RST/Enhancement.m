function Enhancement(data_dir, video_name, video_type, segPath, description, params, inputFolder, outputFolder)
...
if (~isempty(description))
    ...
    loadDir = [params.saveDir '/' params.dataset '/' inputFolder '/' video_name '/' contrast];
end
...
if (~isempty(description))
    ...
    saveDir = [params.saveDir '/' params.dataset '/' outputFolder '/' video_name '/' contrast];
end
...
for iF = 1 : nF
    ...
    for ind = 1:length(pngOutputName)
        %try
            ...
            So{ind} = ImageEnhancement(Si{ind}, params.method, params);
