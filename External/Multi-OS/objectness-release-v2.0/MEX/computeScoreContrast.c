     ...
     
{
    ...
    for(w=0;w<numberWindows;w++)
    {
       ...
       xminSurr=ceil(MAX(xmin[w]-offsetWidth,1));
       xmaxSurr=ceil(MIN(xmax[w]+offsetWidth,width));
       yminSurr=ceil(MAX(ymin[w]-offsetHeight,1));
       ymaxSurr=ceil(MIN(ymax[w]+offsetHeight,height));
